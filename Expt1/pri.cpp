#include<bits/stdc++.h>
using namespace std;
void srt(int n, vector<int> &process, vector<int> &burst_time, vector<int> &priority){
    int temp,pos;
    for(int i=0;i<n-1;i++){
        pos = i;
        for(int j=i;j<n;j++){
            if(priority.at(j)<priority.at(pos)){
                pos = j;
            }
        }
...
int main(){
    int n,p,b,pr;
    vector<int> process, burst_time, cmplt_time, priority,wait_time;
    cout << "Enter the number of processes: ";
    cin >> n;
    for(int i=0;i<n;i++){
        cout << "Enter process number and burst time and priority: ";
        cin >> p >> b >> pr; 
        process.push_back(p);
        burst_time.push_back(b);
        priority.push_back(pr);
    }
    srt(n,process,burst_time,priority);
